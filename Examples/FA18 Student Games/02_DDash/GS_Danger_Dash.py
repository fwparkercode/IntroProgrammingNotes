...
jump_tutorial = big_font.render("Button B = Jump", True, BLACK)
...
dash_tutorial = big_font.render("Button A = Dash", True, BLACK)
title = big_font.render("Welcome to Danger Dash!", True, BLACK)
press_spacebar = small_font.render("Press the A to continue.", True, WHITE)
...
# Images
sky = pygame.image.load("skybox.png").convert()
sky.set_colorkey(BLACK)

far_mountain = pygame.image.load("far_mountain.png").convert()
far_mountain.set_colorkey(BLACK)

close_mountain = pygame.image.load("close_mountain.png").convert()
close_mountain.set_colorkey(BLACK)

treeline = pygame.image.load("trees.png").convert()
treeline.set_colorkey(BLACK)

background = pygame.Surface([680, 480])
background.blit(far_mountain, [0, 0])
background.blit(close_mountain, [0, 0])
background.blit(treeline, [0, 0])
...
while not done:
    ...
    for event in pygame.event.get():  # User did something
        ...
        if not death:
            if event.type == pygame.KEYDOWN:
                if event.key == pygame.K_ESCAPE:
                ...
                elif event.key == pygame.K_LEFT:  # Run left
                    ...
                    if player.action == "rise" or player.action == "fall":
                        player.direction = "R"
                        player.change_x = 3
                elif event.key == pygame.K_k:  # Jump
                    ...
                    if player.action == "stand" or player.action == "run":
                        player.action = "duck"
                elif event.key == pygame.K_j:  # Dash
                    key_pressed += 1
    ...
    if win:
        ...
        background_music.fadeout(1240)
        screen.fill(WHITE)
        screen.blit(win_game_over, [150 // 2, 180 // 2])
