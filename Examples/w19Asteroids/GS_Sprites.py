...
class Player(pygame.sprite.Sprite):
    ...
    def __init__(self, x, y):
        ...
        self.radius = 6
        self.length = 6  # referenced in pygame drawing
        ...
        self.thrust = 0.03  # how much speed we add
        ...
        self.thrust_sound = pygame.mixer.Sound(
            "thrust.wav")
    ...
    def update(self):
        ...
        if not self.active:
            ...
            for asteroid in self.asteroid_sprites:
                dist = math.hypot(self.rect.centerx - asteroid.rect.centerx, self.rect.centery - asteroid.rect.centery)
                dist -= self.radius
                dist -= asteroid.radius
                if dist < 24:
                    return
            else:
                self.active = True
    ...
    def draw_me(self, screen):
        ...
        #pygame.draw.line(screen, YELLOW, self.nose, self.tail1, linewidth)
        #pygame.draw.line(screen, YELLOW, self.nose, self.tail2, linewidth)
        #pygame.draw.line(screen, YELLOW, self.tail1, self.tail2, linewidth)
        pygame.draw.polygon(screen, WHITE, [self.nose, self.tail1, self.tail2], linewidth)
...
class Asteroid(pygame.sprite.Sprite):
    def __init__(self, radius, player):
        ...
        self.change_x = random.random() - 0.5
        self.change_y = random.random() - 0.5

        # spawn to left or right of palyer
        if random.randrange(2):
            self.rect.centerx = player.rect.centerx - random.randrange(int(screen_width / 3), int(screen_width / 2))
    ...
    def update(self):
        ...
        self.rect.centerx = self.x
        if self.rect.left > screen_width:
            self.rect.right = self.rect.left % screen_width
            (self.x, self.y) = self.rect.center

        if self.rect.right < 0:
            self.rect.left = screen_width + self.rect.right
            (self.x, self.y) = self.rect.center
...
class BrokenParts(pygame.sprite.Sprite):
    def __init__(self, point1, point2, change_x, change_y):
        ...
        self.angle = 0
        self.angle_speed = random.random()/2 - 0.5
        self.length = 2
...
#         self.y = y
#         self.size = size
#         self.particle_list = [[x , y]]
#         self.spray_list = []
#         for i in range(random.randrange(int(size / 2))):
#             y = y
#             x = x
#             direction = random.random() * 2 * math.pi
#             speed = random.random() * 10
#             health = random.randrange(30)
#             self.particle_list.append([x, y, direction, speed, health])
#
#     def update(self):
#         for i in range(len(self.particle_list)):
#             self.particle_list[4] -= 1
#             if self.particle_list[]
#             flake_list[i][0] += flake_list[i][2] * math.cos(flake_list[i][3])
#             flake_list[i][1] += flake_list[i][2] * math.sin(flake_list[i][3])
#             flake_list[i][2] *= 1.02
#             flake_list[i][3] += 0
#             if flake_list[i][1] > height or flake_list[i][1] < 0:
#                 flake_list[i][3] = random.random() * 2 * math.pi
#                 flake_list[i][2] = random.random() * 5 + 0.5
#                 flake_list[i][1] = height / 2
#                 flake_list[i][0] = width / 2
#             if flake_list[i][0] > width or flake_list[i][0] < 0:
#                 flake_list[i][0] = width / 2
#                 flake_list[i][1] = height / 2
#                 flake_list[i][3] = random.random() * 2 * math.pi
#                 flake_list[i][2] = random.random() * 5 + 0.5
...
#             pygame.draw.line(screen, WHITE, particle[0], particle[1])
#
# class Particle()
#     def __init__(self):
