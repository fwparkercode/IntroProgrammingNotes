...
Aaron Lee - 2019
"""
from sprite_classesTINY0 import *
...
class Game():
    ...
    def initialize_pygame(self):
        ...
        self.game_font = pygame.font.Font("Hyperspace Bold.otf", 10)
        self.power_font = pygame.font.Font("Hyperspace Bold.otf", 4)

        self.fire_sound = pygame.mixer.Sound("fire.wav")

        self.thrust_sound = pygame.mixer.Sound("thrust.wav")

        self.bg = pygame.mixer.Sound("bg.wav")
        self.bg.play(-1)

        self.life_sound = pygame.mixer.Sound("positive.wav")
    ...
    def initialize_level(self):
        ...
        for i in range(self.level):
            asteroid = Asteroid(random.randrange(40, 50 + self.level * 10))
    ...
    def ars(self):
        for star in self.star_list:
            pygame.draw.line(self.screen, WHITE, star, star, linewidth)
    ...
    def make_babies(self, asteroid):
        for i in range(random.randrange(2, 4)):
            baby = Asteroid(int(asteroid.radius * (random.random()/5 + 0.4)))
    ...
    def draw_gui(self):
        ...
        if self.player.shield_time > 10 or self.frame % 30 > 10:
            ...
            for x in range(my_shield.get_width() + 20, my_shield.get_width() + 20 + int(self.player.shield_time), 5):
    ...
    def draw_stars(self):
        for star in self.star_list:
            pygame.draw.line(self.screen, WHITE, star, star)
...
def intro_screen(screen, clock):
    ...
    my_font = pygame.font.Font(
        "Hyperspace Bold.otf", 80)
    small_font = pygame.font.Font(
        "Hyperspace Bold.otf", 20)
    while not done:
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                done = True
        ...
        centered_text(screen, my_font, "Asteroids", WHITE, 0)
        centered_text(screen, small_font, "Press any key to continue", WHITE, 90)
        centered_text(screen, small_font, "Aaron Lee 2019", WHITE, 120)
