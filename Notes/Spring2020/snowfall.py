...
for i in range(500):
    flake_x = random.randrange(WIDTH - 5)
    flake_y = random.randrange(HEIGHT - 5)
...
while not done:
    ...
    for flake in flake_list:
        ...
        if flake[1] > HEIGHT:
            flake[1] = -5
            flake[0] = random.randrange(WIDTH - 5)

    # --- Drawing code should go here
    screen.fill(BLACK)  # paint the blank canvas

    for flake in flake_list:
        pygame.draw.ellipse(screen, WHITE, [flake[0], flake[1], 4 + flake[2], 4 + flake[2]])
